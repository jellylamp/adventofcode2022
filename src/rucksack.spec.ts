...
import {RockPaperScissors} from "./paper-rock-scissors";
...
CrZsJsPPZsGzwwsLwLmpwMDw`;
  ...
  const score = Rucksack.getRucksackCount(input);
  expect(score).toEqual(157);
});

test("Day 3 puzzle b sample", () => {
  const input =
`vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
...
CrZsJsPPZsGzwwsLwLmpwMDw`;
  ...
  const score = Rucksack.getGroupId(input);
...
vjWPWjWPPPWgwmfCrNvTvZ`;
  ...
  const score = Rucksack.getRucksackCount(input);
  expect(score).toEqual(8153);
  const score2 = Rucksack.getGroupId(input);
  expect(score2).toEqual(2342);
});
